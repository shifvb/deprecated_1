...
from look_labels_app.utils.SUV_calculation.SUVTools import getASuv
...
def get_registered_suv_and_hu_array(ct_series_dir: str, pt_series_dir: str):
    ...
    hu_filenames = [_ for _ in os.listdir(ct_series_dir) if _.startswith("CT_")]
    hu_filenames.sort(key=lambda filename: int(filename.split("_")[-1]))
    hu_images = sitk.ReadImage(hu_filenames)
    os.chdir(pt_series_dir)
    suv_filenames = [_ for _ in os.listdir(pt_series_dir) if _.startswith("PT_")]
    suv_filenames.sort(key=lambda filename: int(filename.split("_")[-1]))
    suv_images = sitk.ReadImage(suv_filenames)
...
def _get_ratio(pt_series_dir: str):
    ...
    :return: k的值
    """
    pt_file_name = [_ for _ in os.listdir(pt_series_dir) if _.startswith("PT_")][0]
