...
def getBaseInfo(dicom_dir_path=None):
    ...
    if dicom_dir_path is not None:  # 从第一个文件中获取患者信息
        _filenames = [_ for _ in os.listdir(dicom_dir_path) if _.startswith("PT_") or _.startswith("CT_")]
        _filename = os.path.join(dicom_dir_path, _filenames[0])
        _genBaseInfo(dicom.read_file(_filename))
    """[INFO] code added end"""
    return baseInfo
...
def _calSuv(pixels, slopes):
    ...
    return np.array(suvLbm)


def getSUVs(path, filepattern):
    '''
    计算给定路径下所有PET图像的SUV值
...
def getRegistedSUVs(pixels, ptPath):
...
def testBigSUV():
    ...
    return getRegistedSUVs(ctp, ptp)
