...
from look_labels_app.utils.SUV_calculation import getBaseInfo
from look_labels_app.gui import CoronalPlaneGUI, TransversePlaneGUI
from look_labels_app.utils.load_data import load_data
...
class MainGUI(object):
    ...
    def load_coronal_plane(self):
        """加载冠状面病人图像"""
        CoronalPlaneGUI(self.ct_arrs, self.suv_arrs, self.patient_info_ct,
                        self.patient_info_pt) if self.is_loaded else None

    def load_transverse_plane(self):
        """加载横断面病人图像"""
        TransversePlaneGUI(self.ct_arrs, self.suv_arrs) if self.is_loaded else None
